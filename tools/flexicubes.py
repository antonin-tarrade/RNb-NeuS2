...
class FlexiCubes:
    ...
    def construct_voxel_grid(self, res):
        ...
        # verts = (self.cube_corners.unsqueeze(0) / res + coords.unsqueeze(1)).reshape(-1, 3) original
        verts = []
        chunk_size = 16384  # Process 16k coordinates at a time
        for i in range(0, coords.shape[0], chunk_size):
            chunk = coords[i:i+chunk_size]
            chunk_verts = (self.cube_corners.unsqueeze(0) / res + chunk.unsqueeze(1)).reshape(-1, 3)
